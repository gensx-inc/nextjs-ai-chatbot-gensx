...
export function createGSXComponent<TFn extends (...args: any[]) => any>(
  ...
  return gensx.Component<Parameters<TFn>[0], Awaited<ReturnType<TFn>>>(
    name,
    (props) => {
      // Make sure to bind the function to its original context if needed
      return fn(props);
    },
  );
}
...
> = gensx.Component("StreamText", (props) => {
  // Directly call the ai.streamText function with the props
  return ai.streamText(props);
});
